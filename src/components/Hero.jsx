...
const Hero = () => {
  ...
  const {picupDate, setPickupDate, returnDate, setReturnDate, navigate} =
  useAppContext()

  const handleSearch = (e)=>{
      e.preventDefault() 
      navigate ('/cars?pickupLocation=' + pickupLocation + '&pickupDate=' +
          picupDate +  '&returnDate=' + returnDate)  
  }
       ...
       justify-between p-6 rounded-lg md:rounded-full w-full max-w-80 md:max-w-200
        ...
        <div className='flex flex-col md:flex-row items-start md:items-center gap-10 md:ml-8'>
          ...
          <div className='flex flex-col items-start gap-2'>
            ...
            <input value={picupDate} onChange={e=>setPickupDate(e.target.value)} type='date' id='pickup-date' min={new Date().toISOString().split('T')[0]} className='text-sm text-gray-500' required />
          </div>
